...
- D램 현물 가격(DRAMeXchange) — 매일 누적 중이며, 이력이 검증에 충분해지기 전까지는 표시만
- 내재변동성(VXN) 구간 — 실적·FOMC 앞에서 옵션 시장이 보는 변동폭만큼 종가 예측 구간을 넓히는 후보. 원장에 `Candidate IV interval`로 채점하며 이벤트일 적중률이 판단 기준
- 09:37 **시가 반영 갱신** — 확정된 시가를 반영한 종가 방향을 07:00 예측 아래 따로 보임. 더 나은 모델이 아니라 늦은 정보 시점이며, 별도 행(`Post-open`)으로 따로 채점
...
운영자용으로 `/lab/`에 **실험실** 페이지가 있습니다(가상 매매 시뮬레이션, 무엇이 예측을 밀었나)(랜딩에 링크하지 않으며 admin 페이지에서 들어갑니다). 원장에 실제로 기록한 예측만으로 09:00 시가 매수 → 15:30 종가 청산을 돌리고, **보유 전략과 나란히** 보여 줍니다. 수수료·거래세·슬리피지는 끌 수 없습니다. **무엇이 예측을 밀었나** 탭은 그날 모델에서 뽑은 특징 기여도(LightGBM 트리 기여도, 로지스틱 계수×표준화값 — 새 의존성 없음)를 `forecast_history/<종목>/attribution.csv`에 하루 한 줄씩 쌓아 보여 줍니다. 기여도가 큰 특징이 도움이 된 특징은 아니므로, 맞힌 날과 틀린 날을 갈라 보는 화면을 함께 두되 표본이 60일에 못 미치면 잡음이라고 적습니다. 예측력이 갭 구간에 있으므로 이 시뮬레이션이 보유를 이기지 못하는 것이 정상이고, 그 사실을 금액으로 확인하는 것이 목적입니다.

사전 예측은 **06:00~09:00 KST 실행만** 원장에 기록합니다. 미국 정규장이 끝난 뒤여야 밤사이 정보가 들어가고, 한국 개장 전이어야 사전 예측입니다. 한국 저녁(17:00~23:00) 실행은 같은 예측일을 `Candidate evening forecast`로 **나란히** 기록해, 두 시점의 적중률 차이로 **밤사이 미국 시장 정보의 값**을 직접 잽니다(갭 AUC 0.80의 검증). 덮어쓰지 않는 이유는 원장이 날짜별 최초 사전 예측만 집계하기 때문입니다 — 나중에 갱신하면 결과를 보고 예측을 고른 것과 구별할 수 없습니다.
