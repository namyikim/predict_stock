...
운영자용으로 `/lab/`에 **가상 매매 시뮬레이션** 페이지가 있습니다(랜딩에 링크하지 않으며 admin 페이지에서 들어갑니다). 원장에 실제로 기록한 예측만으로 09:00 시가 매수 → 15:30 종가 청산을 돌리고, **보유 전략과 나란히** 보여 줍니다. 수수료·거래세·슬리피지는 끌 수 없습니다. 예측력이 갭 구간에 있으므로 이 시뮬레이션이 보유를 이기지 못하는 것이 정상이고, 그 사실을 금액으로 확인하는 것이 목적입니다.
