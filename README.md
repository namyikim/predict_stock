...
| 워크플로 | 시각(KST) | 하는 일 |
| --- | --- | --- |
| 일일 보고서 | 평일 06:22 (+ 07:25 백업) | 그날의 **확정 예측**을 만들어 원장에 기록하고 보고서를 발행합니다. 전날 밤 미국 시장 정보가 들어 있습니다. GitHub의 cron은 `:00`·`:30`에 요청이 몰려 크게 밀리거나 통째로 건너뛰므로(2026-09-08 06:30 예정 실행은 실행 흔적조차 없었습니다) 붐비지 않는 분에 걸고 한 시간 뒤 백업을 하나 더 둡니다. 백업은 오늘 사전 예측이 원장에 이미 있으면 아무것도 하지 않고 끝냅니다(`tools/should_run_today.py`). 검색어·금은·중국·관심도 보고서는 백업 회차에는 돌지 않습니다. |
