...
참조월+1개월 20일 이후 값만 쓰지만 개정 문제는 남아 있어 백테스트는 낙관적입니다. FRED 인증키(`FRED_API_KEY`, 무료)가 필요하고 시리즈 코드는 `FRED_CLI_SERIES_ID`(기본 `G20LOLITOAASTSAM`)로 바꿀 수 있습니다. 코드 확인은 `python tools/probe_fred.py`.
