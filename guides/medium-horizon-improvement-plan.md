...
상태: M00~M06 완료, M07 후보 등록·잠금 평가 완료(관찰 중), M08 현행 유지·관찰 연장 결정 기록(2026-09-10). 운영 채택 없음.
...
| M08 | [ ] | 종목·지평별 채택 또는 유지·복구 | M07 | 결정 기록 완료: 4조합 모두 유지, 하이닉스 3후보 관찰 연장(운영 채택 아님). 복구값·회귀 결과·실행 환경 기록. 관찰 판정 뒤 상위 체크 |
...
- [x] 4절 기준으로 네 조합과 목적별 채택/유지/관찰 연장을 기록한다. 가격·방향·구간 채택을 분리한다. → `experiments/medium_horizon/M08/decision.md` 1절
- [x] 채택할 때만 기본 설정을 변경하고 이전 모델·특징·보정·원장 설정을 복구값으로 저장한다. → 기본 설정 변경 없음. 코드 변경 4건의 복구 방법과 채택 시 바꿀 값·복구값 기록(2절)
- [x] 노트북 단독 실행, 보고서 표현, 최초 예측 보존, 후보 장애 fallback, 전체 회귀 테스트를 수행한다. → quick 단독 실행 성공, 553개 테스트(환경 오류 1, CI 통과), 3절
- [x] 모델별 실행 시간·메모리와 기존 자동 실행 환경 적합성을 기록한다. → 4절(노트북 캐시 8분/종목 1회, 이후 작업 10~25초, Actions 영향 수 초)
...
| 2026-09-10 | M08 | 조합·목적별 결정(4조합 유지, 하이닉스 3후보 관찰 연장), 복구값, 회귀 553개(환경 오류 1), 실행 시간 기록. `experiments/medium_horizon/M08/decision.md` | 노트북 quick 단독 실행 성공 | M07 첫 점검(2026-12/2027-03) 후 M07·M08 상위 체크와 결정 갱신 |

- 현재 작업: M07 관찰(코드 변경 없음). 다음 실행: 2026-12 이후 `--task M07 --target sk_hynix` 로 현황 확인 → 첫 점검 판정 → M07·M08 갱신.
- 완료: 7/9 상위 체크(M07·M08 은 관찰 판정 뒤). 하위 항목은 M07 4~6 을 빼고 모두 완료. 보류: 없음. 실제 후보 채택: 없음. 관찰 후보 3(하이닉스). M07 후보 예정: sk_hynix 5·20일 HAR 구간(구간 목적), sk_hynix 5일 3/3 발행 판정(가격 목적). 관찰 대상: full_plus_A(samsung/20, 보정 후 −0.045%p, 탐색 결과).
- 먼저 읽을 파일: 본 문서, `experiments/medium_horizon/M08/decision.md`, M07 decision.md, M06~M00 decision.md, `tools/run_medium_horizon.py`(load_inputs·evaluation_folds·analyse_horizon), forecast_utils.py의 `price_design_frame`·`calibrate_price_forecast`.
...
- 마지막 검증: `python -m unittest discover -s tests` 553개(환경 오류 1: test_easy_summary cp949, CI 통과). M07 full 두 종목(exit 0, purge 0). 노트북 quick(sk_hynix, 캐시) 실행으로 후보 행 3개 생성 확인.
