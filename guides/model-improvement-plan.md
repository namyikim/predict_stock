...
**현재 상태:** 계획 작성. 아래 신규 작업은 아직 실행하지 않음.
**첫 실행:** P00만 진행.

## 1. 운영 규칙과 전역 제약

- 한 대화에서는 작업 ID 하나만 수행한다. 토큰이 부족하면 해당 작업의 완료한 하위 항목과 재개 지점을 기록하고 멈춘다.
...
| --- | --- | --- | --- | --- | --- |
| P00 | [ ] | 현행 기준선·평가 계약 고정 | 없음 | CPU, 전체 평가는 Colab | 미실행 |
...
- 다음 작업: P00
- 보류 작업: 없음
- 마지막 검증 결과: 계획 문서 검토만 수행; 모델 학습·백테스트 미실행
