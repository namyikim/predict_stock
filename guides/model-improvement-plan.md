...
### P16 — 시가 확정 후(09:37) 종가 방향 재예측 (계획 밖 등록 2026-09-20)

**파일:** 러너 P16 등록(`run_p16`·`p16_gap_features`·`gap_rule_labels`·`gap_rule_probabilities`), `tests/test_post_open_reforecast.py`.
**입출력:** P00 고정 스냅샷과 같은 12폴드 → 현행 07:00 특징(`t0700`)과 같은 특징 + 실현 갭 6열(`t0900`)을 같은 날짜에서 쌍체 비교.

P03·P09·P10b·R02c 는 모두 “07:00 특징에 무엇을 더할까”를 물었고 전부 동률 또는 열위였다.
P16 은 **모델이 아니라 정보 마감 시각**을 옮긴다. 09:00 시가가 확정되면 `close_to_close = (1+gap)(1+session)−1`
의 갭 성분이 예측 대상이 아니라 **관측값**이 된다. 일봉에 `open` 이 이미 있으므로 새 수집이 필요 없다.

- [x] 행 d 의 당일 입력이 **시가 하나뿐**임을 기계로 고정한다(종가·고가·저가·거래량 불변, 미래 봉 불변, 예측일 봉 없으면 결측). → `tests/test_post_open_reforecast.py` 18개
- [x] `t0700` 이 노트북 대표 모델 OOF 를 정확히 재현하는지 확인한다. → 두 종목 **argmax 일치율 1.000, log_loss 차이 0.000000**
- [x] 그룹 G 결측·시가 없는 날은 두 후보에서 함께 빼 쌍체를 유지하고 건수를 기록한다. → 그룹 G 결측 1행(첫 봉, 폴드 밖) · 시가 없는 평가일 0일 → **12폴드 전부 제외 0**
- [x] 모델 없는 `gap_rule`(갭 부호만 읽는 규칙)을 같은 날짜에서 함께 채점해 모델의 몫을 분리한다. → 확률은 학습 구간 버킷별 조건부 분포
- [x] 세션(시가→종가) 타깃을 2순위 진단으로 같이 돌려 “관측된 갭이 남은 장중을 설명하는가”를 재다. → 여덟 칸 전부 동률

#### 결과 (2026-09-20, `experiments/model_improvement/P16/20260920T0000Z_post_open_reforecast/`)

평가 samsung 1,367일 / sk_hynix 1,362일(두 후보 동일). 입력 61→67 / 58→64열.

| 종목 | 모델 | 정보 마감 | accuracy | balanced_accuracy | log_loss |
| --- | --- | --- | ---: | ---: | ---: |
| samsung | t0700(현행 대표) | 07:00 | 0.4543 | 0.4329 | 1.0285 |
| samsung | gap_rule(모델 없음) | 09:00 | 0.5443 | 0.5424 | 0.9617 |
| samsung | t0900(+실현 갭) | 09:00 | 0.5640 | 0.5398 | 0.9258 |
| sk_hynix | t0700(현행 대표) | 07:00 | 0.4993 | 0.4652 | 1.0146 |
| sk_hynix | gap_rule(모델 없음) | 09:00 | 0.5441 | 0.5384 | 0.9411 |
| sk_hynix | t0900(+실현 갭) | 09:00 | 0.5756 | 0.5427 | 0.9145 |

t0900 − t0700 (95% 월 블록 CI, log_loss 는 음수가 개선):
samsung log_loss **−0.10269 [−0.12361, −0.08178]** · balanced_accuracy **+0.10694 [+0.08059, +0.13499]**;
sk_hynix log_loss **−0.10011 [−0.12280, −0.07910]** · balanced_accuracy **+0.07750 [+0.05146, +0.10504]**.
t0900 − gap_rule log_loss samsung −0.03586 [−0.04904, −0.02094] · sk_hynix −0.02666 [−0.04023, −0.01340](둘 다 유의 우위),
balanced_accuracy 는 두 종목 모두 **동률**.

**세션 타깃(2순위)은 여덟 칸 전부 동률이다.** log_loss samsung −0.00163 [−0.00393, +0.00098] ·
sk_hynix −0.00101 [−0.00218, +0.00017], 세션 AUC 차이 −0.002 / +0.002.
**실현 갭을 보여 줘도 남은 장중 방향은 조금도 더 맞히지 못한다** — 종가→종가의 이득은 전부 관측된 갭이다.

**채택 조건은 충족했으나 이것은 모델 개선이 아니라 정보 시점 차이다.** 0.45 와 0.56 은 서로 다른 질문의
답이므로 한 표에 넣을 때는 정보 마감 시각을 반드시 병기한다. 09:37 재예측은 시가에 행동하려는 사람에게는
...
### P18 — 전날 저녁(20:00 KST) 예측에 저녁 거래 자산을 더할 수 있는가 (계획 밖 등록 2026-09-20)

**파일:** 러너 P18 등록(`run_p18`·`evening_feature_frame`·`evening_column_sessions`·`evening_futures_features`·
`futures_return_to_cutoff`·`index_session_return_to_cutoff`·`p18_fill_policy`·`p18_ledger_check`·`ensure_hourly_cache`),
`tests/test_evening_futures.py`(22개).
**입출력:** P16 과 같은 고정 스냅샷·12폴드 + yfinance 시간봉(NQ=F·ES=F·^GDAXI·^STOXX50E, `interval=1h`, `period=730d`,
`runs/model_improvement/P18/cache/`) → 시간봉이 있는 같은 날짜에서 다섯 모델 쌍체 비교.

**물음.** 저녁 실행(`Candidate evening forecast`, 셀 1 `EVENING_WINDOW_KST=(17,23)`)은 별도 모델이 아니라 아침 모델을 그 시각의
일봉으로 돌린 것이라 해외 열이 전부 직전 미국 세션 값이다. 원장 5일에서 아침 42%/55% vs 저녁 40%/20% 였다. 20:00 KST 에
실제로 움직이는 CME 야간 선물·유럽 지수를 더하면 나아지는가. 정직한 기대는 "기껏해야 작은 이득"이었다.

- [x] **`t_evening` 재구성**: 한국(`sam_`·`kospi_`·`peer_`)·달력(`cal_`) 열은 아침 행 d, us/cont/london 열은 아침 행 d−1,
  `gdr_overnight_signal` 은 gdr(d−1)−sam(d). 세션 표는 노트북 셀 11 `ASSET_SESSION` 을 ns 에서 읽고 모르는 열은 예외.
  `feat` 에서 다시 만든 아침 행렬이 노트북 `market_X` 와 최대 차이 0.0.
- [x] **후보 `t_evening_f2000`**: 전 한국 세션 날짜 20:00 KST 까지 **끝난** 시간봉만으로 6열(`nq_ret`·`nq_sign`·`nq_cum5`·
  `es_ret`·`dax_ret`·`stoxx_ret`) + 가용 플래그. 선물 기준은 16:00 ET 정규 마감 봉(18:00 ET 이후 야간 봉은 기준 아님), 유럽은
  그 날짜 첫 봉 시가. 시간봉 없는 학습 행은 0 + 플래그 0. 누수 22개 테스트(마감 걸친 봉·야간 봉·서머타임·월요일·유럽 휴장).
- [x] **민감도 `t_evening_f2200`**(22:00 KST, 미국 개장 직전) · 참조 `t0700`(아침 상한) · `nq_rule`(모델 없는 선물 부호 규칙,
  확률은 학습 구간 버킷).
- [x] 평가일 = 시간봉 범위 ∩ 학습 구간 선물 가용 행 ≥ 100 인 폴드(8~11) 시험일: **두 종목 394일, 2025-01-02 ~ 2026-09-16**,
  다섯 모델 같은 날짜. 실제 원장 저녁 행 3일과 대조(argmax 일치 1/3 — 저녁 확률이 사전확률 근처라 학습 창 차이로 뒤집힌다).

#### 결과 (2026-09-20, `experiments/model_improvement/P18/20260920T0500Z_evening_futures/`)

| 종목 | 모델 | 정보 마감 | accuracy | balanced_accuracy | log_loss | 갭 AUC |
| --- | --- | --- | ---: | ---: | ---: | ---: |
| samsung | t_evening | 20:00 d−1 | 0.3655 | 0.3601 | 1.0930 | 0.473 |
| samsung | t_evening_f2000 | 20:00 d−1 | 0.3528 | 0.3548 | 1.0941 | 0.486 |
| samsung | t_evening_f2200 | 22:00 d−1 | 0.3553 | 0.3585 | 1.0928 | 0.489 |
| samsung | t0700 | 07:00 d | 0.4467 | 0.4361 | 1.0324 | 0.807 |
| sk_hynix | t_evening | 20:00 d−1 | 0.3350 | 0.2932 | 1.0979 | 0.452 |
| sk_hynix | t_evening_f2000 | 20:00 d−1 | 0.3629 | 0.3207 | 1.0970 | 0.499 |
| sk_hynix | t_evening_f2200 | 22:00 d−1 | 0.3883 | 0.3457 | 1.0950 | 0.528 |
| sk_hynix | t0700 | 07:00 d | 0.4898 | 0.4529 | 0.9966 | 0.824 |

f2000 − t_evening log_loss **samsung +0.0011 [−0.0015, +0.0041] · sk_hynix −0.0010 [−0.0049, +0.0032]**, f2200 도 동률
(−0.0002 / −0.0029). balanced_accuracy 는 sk_hynix f2200 만 +0.053 [+0.015, +0.091] 우위, 나머지 동률. 갭 AUC 는
f2000 +0.013 [+0.003, +0.026] / +0.047 [+0.026, +0.070] 우위 — 실재하지만 확률로 바뀌지 않는 크기.
t0700 까지 남은 거리는 선물을 더해도 그대로다: log_loss samsung +0.062 [+0.030, +0.095] · sk_hynix +0.100 [+0.065, +0.137].
`nq_rule` 은 t_evening 과 전부 동률이고 후보는 규칙 위에 확률 품질로 더하는 것이 없다.

**채택 없음(사전 규칙: log_loss CI 상한 < 0 — 네 칸 전부 불충족).** 저녁 기준선은 사전확률 수준이다 — 원장 5일 표본은
노이즈였고, 저녁 예측이 나쁜 것은 모델 탓이 아니라 20:00 에 답을 만드는 정보가 없기 때문이다. P16 이 정보를 뒤로 옮겨
+0.10 을 얻었듯 여기는 앞으로 옮겨 −0.06/−0.10 을 잃는다. 후보가 선물 열을 158~499행에서만 배웠다는 한계는 재실험
(시간봉 3년 이상 쌓인 뒤) 근거이지 채택 근거가 아니다. 운영 반영했다면 저녁 실행이 yfinance 시간봉을 실행 시각에 받아야
했고(730일 제한·간헐 실패·봉 라벨 규칙 미문서) 실제 실행 시각을 마감으로 원장에 적어야 했다 — 감수할 이득이 없다.
코드·노트북·보고서·원장 무변경.

### P17 — 해외 수익률의 as-of 결합과 결측 채움 정리 (계획 밖 등록 2026-09-20, 완료·채택 2026-09-24)

**출처.** 2026-09-20 코드 검토(`reports/2026-09-20-code-data-economic-review.md`) #6. 노트북 셀 15 의
`merge_latest_available` 는 해외 자산의 '수익률'을 최근 값으로 as-of 결합하므로 미국 휴장일에는 직전 거래일의
수익률이 다음 한국 거래일에도 그대로 반복된다. 그 뒤 `feat.ffill(limit=5)` 는 `safe_pct_change` 가 결측으로
드러낸 값까지 다시 채운다. 코드로 확인했다(사실). 미국 휴장과 API 실패가 같은 값으로 보인다.

**왜 바로 고치지 않는가.** 대표 모델의 입력 분포가 바뀌는 변경이라 '버그 수정'으로 밀어 넣으면 P00 이후의
모든 비교가 무효가 된다. 계획 4절대로 같은 12폴드·같은 날짜에서 쌍체 CI 로 재고, 이기거나 동률이면서 정직한
쪽을 채택한다. 동률이어도 정직한 정의(반복된 값이 새 정보로 보이지 않는 것)를 택할 근거는 된다.

- [x] 후보: (a) 하나만 돌렸다 — 가격 수준 as-of + 한국 행 사이 누적, `us_new_session`·`us_obs_age_days`. (b) 는 (a) 의
  부분집합(휴장일 0)이라 후보를 늘리지 않았다(반복 비교 편향).
- [x] 누수·정직성 검사: 새 미국 세션이 없는 88행에서 후보 값이 모두 0(지금 값은 88행 모두 직전 반복). API 실패는
  7일 허용을 넘으면 결측 → 라이브 대체 경고로 드러나, 휴장(0·flag 0)과 구분된다.
- [x] 판정: P03 계약. `experiments/model_improvement/P17/20260924T0526Z_asof_cumulative/decision.md`.

### P19 — 대표 모델 학습 행을 시세 열 기준으로 (계획 밖 등록 2026-09-24, 완료·채택 없음)

**출처.** 2026-09-20 코드 검토 #7(그때 미조치). 노트북은 모든 특징이 있는 행만 남긴 뒤 시세 열을 떼어 대표 모델을
학습하므로, 대표 모델이 쓰지 않는 보조 자료의 결측도 학습 행을 줄인다(사실).

- [x] 후보 하나: 시세 열·정답·밴드가 있는 행으로 학습, 폴드는 같은 규칙으로 재생성, 평가는 공통 날짜.
- [x] 결과: 잘린 행은 samsung 4 / sk_hynix 6(0.2%). log_loss 동률(+0.0011 / +0.0004). samsung 정확도 +1.2%p 유의는
...
| 2026-09-20 | 코드 검토 조치 | `reports/2026-09-20-code-data-economic-review.md` 의 10건 중 8건 조치·1건 실험 등록(P17)·1건 미조치(#7). 커밋 `9b8a8e95`(#1 장기 보정 누수, 캐시 덮어쓰기, tmp 경로), `8db59082`(#4 conformal 구간), `655ee4a4`(#2·#3 단가·물량), 이 문서를 갱신한 커밋(#5 시세만 모델 상시 생성, #8 같은 분기 비교, #9 VXN 주석, #10 달력 기준 앞 k개월) | 각 묶음마다 새 테스트 + 전체 회귀 통과 | 10건 모두 코드로 사실 확인. #6 은 대표 모델 입력 변경이라 P17 로 등록 |
...
- 후속 항목(계획 밖) P18 — 저녁 예측 + 저녁 거래 자산: **완료(2026-09-20), 채택 없음.** 20:00 KST 에 있는 정보(선물·유럽)는 다 넣었고 확률 품질은 0 이다. 저녁 실행은 사전확률 수준의 대조군으로만 의미가 있다. 시간봉 3년 이상 쌓인 뒤 재실행 가능
- 완료한 신규 작업: 11/16 (P00~P10) + 계획 밖 P10b·R02c·P16·P18. 보류 4 (P11~P14). 진행 중 1 (P15)
