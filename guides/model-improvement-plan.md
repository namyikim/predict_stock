...
### P17 — 해외 수익률의 as-of 결합과 결측 채움 정리 (계획 밖 등록 2026-09-20, 미실행)
...
- [ ] 후보: (a) 가격 수준을 as-of 결합하고 수익률은 한국 예측 시각 사이의 누적으로 새로 계산, `new_session`
  플래그와 마지막 관측 나이를 특징으로 둔다. (b) 최소 변경 — 휴장일의 반복 수익률만 0 으로 두고 나이 특징 추가.
- [ ] 누수·정직성 검사: 휴장 다음 날 행의 해외 수익률이 직전 값과 같지 않을 것, API 실패와 휴장이 구분될 것.
- [ ] 판정: P03 계약. log_loss·balanced_accuracy 두 종목 쌍체 CI.
- 예상 규모: 반나절. 자료 있음.
