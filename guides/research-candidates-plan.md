...
상태: R07·R09 완료, R08 1·2단계 완료(2026-09-12), R02 완료(2026-09-16, R02c 포함), R01 완료(2026-09-20, 범위 확장: 시초가·종가 1/5/20일). R03~R06 미실행, R08 2b·3단계 미실행.
...
| ID | 완료 | 작업 | 선행 | 결론 |
| --- | --- | --- | --- | --- |
| R01 | [x] | 구간 재보정(현행 q vs 최근 창 분위·conformal·ACI, HAR 참고) — 운영 OOF 위에서 시초가·종가 1/5/20일 × 두 종목 | 없음(M05 코드 재사용) | **시초가만 채택 권고, 나머지 현행 유지(2026-09-20). 운영 미반영.** 여덟 칸 중 채택 조건(Winkler 차 월 블록 CI 상한 < 0 & 포함률 80%±4%p)을 채운 것은 samsung 시초가뿐: `trailing_q`(최근 확정 잔차 250개, 날마다 갱신) 포함률 86.6% → 81.8%, 반폭 2.53% → 2.23%, Δ −0.00198 [−0.00368, −0.00074]. sk_hynix 시초가는 같은 규칙에서 동률(82.4% → 81.7%)이라 kind=open 에만 공통 적용을 권고. sk_hynix 20일(66.5%)은 포함률만 오르고(75.5~80.8%) 반폭 24.9% → 37~47%, Winkler 는 한 번도 개선되지 않음 — 빗나감이 위 31.6%/아래 1.9% 로 쏠려 있어 폭이 아니라 중심값 문제(R04). samsung 20일은 최근 250개 창이 유의 열위(+0.083) → 일괄 교체 불가. ACI 는 어디서도 최근 창 분위를 못 이김. pid·enbpi 는 M05 에 구현이 없어 실행하지 않음 |
| R02 | [x] | 야간/장중 분해 특징·타깃 분해 (+ R02c: 같은 특징을 다음 날 방향 모델에) | 없음(M02 코드 재사용) | 채택 없음(2026-09-16). R02a 그룹 D 18열: samsung/20 만 보정 후 우위(−0.00040 [−0.00076, −0.00010])이나 raw 동률·기울기 반토막·여전히 유지보다 나쁨, 나머지 동률. R02b 분해: samsung/5·20 우위(−0.00005·−0.00037)이나 성분 표 여덟 칸 모두 0 포함 — 5·20일 지평의 야간·장중 어느 성분에도 유지를 이기는 신호 없음. 하이닉스 전부 동률. 관찰 대상으로만 기록. R02c 는 model-improvement-plan 8절 |
...
| R09 | [x] | 선행지수 2~3개월 전망(ARIMA·VAR), 다음 분기 이익 모형에 반영 | 0단계(판본 보관)가 먼저 | 0~3단계 완료(2026-09-12). **채택 없음, 관찰로 남김**. 지수를 OECD 한국으로 바꾸니 2단계는 통과(ARIMA 가 세 지평 모두 기준선에 유의 우위, 3개월 MAE 0.459→0.133). 그러나 3단계에서 두 종목 모두 동률(삼성 −0.310조 [−0.833, +0.157], 하이닉스 −0.075조 [−0.253, +0.055], 공통 21분기). 같은 방법을 G20 에 쓰면 2단계부터 전부 동률이다. 전망 2026-09 102.840 → 11월 102.544 |
...
- 현재 작업: 없음. R01 은 끝났고 운영 반영(시초가 구간만 `band_window=250`)은 **별도 작업으로 남아 있다** — 설계는
  `experiments/medium_horizon/R01/decision.md` "운영 반영 설계". 다음 실험 후보는 R04(중기 종가 구간이 한쪽으로만 빗나가는
  문제 = 중심값·국면)와 R08 2b.
- 2026-09-20 실행: R01 완료(`experiments/medium_horizon/R01/decision.md`, 러너 `--task R01`, 함수 `interval_q_paths`·
  `matured_counts`·`trailing_q_path`·`aci_q_path`·`conformal_quantile`·`open_gap_design`, 테스트
  `tests/test_interval_calibration.py` 19개). **계획과 달리 M05 폴드가 아니라 운영 OOF(노트북 36셀과 같은 분할) 위에서,
  시초가·종가 1일까지 넓혀 쟀다** — 원장 `band_coverage`(시초가 86%·하이닉스 20일 66%)와 같은 숫자를 재현해야 운영 결정을
  내릴 수 있어서다. 계수(W, γ)는 선택 25% 에서만 고르고 평가 25% 로 판정했다. 결과: samsung 시초가만 채택 조건 충족
  (최근 250개 분위, 여덟 설정 모두 통과), 나머지 일곱 칸은 동률이거나 열위. sk_hynix 20일은 q 재보정으로 고쳐지지 않는다
  (빗나감 위 31.6%/아래 1.9%, 포함률 CI [51.6, 81.0] 은 80% 와 구별도 안 된다). samsung 20일에서 최근 250개 창이 유의
  열위라 '모든 칸에 최근 창'은 틀린 처방이다(2026-09-20 에 되돌린 금속 시도와 같은 결론). 금속은 측정하지 않았고 바꾸지 않는다.
  R05(HARQ σ)의 실행 조건 'R01 이 σ 척도 개선을 요구할 때'는 **성립하지 않는다** — HAR σ 는 평가 구간 포함률 55~67% 로 무너졌다.
- 2026-09-16 실행: R02 완료(`experiments/medium_horizon/R02/decision.md`, 러너 `--task R02`, 그룹 D 빌더
  `group_d_features`·성분 라벨 `horizon_leg_labels`, 테스트 `tests/test_overnight_intraday.py` 19개). **채택 없음.**
  R02a(그룹 D 18열)는 samsung/20 만 보정 후 우위(−0.00040 [−0.00076, −0.00010])인데 raw 동률·내부 기울기 0.127→0.060
  반토막·여전히 유지보다 유의하게 나쁨(+0.00035) — M02 full_plus_A 와 같은 "축소가 커져서 손실이 준" 결과라 관찰 대상으로만
  남겼다. R02b(로그 야간 합 + 로그 장중 합, 항등식 1e-15)는 samsung/5·20 우위이지만 성분별 raw 예측력 표 여덟 칸이
  모두 0 을 품는다 — 5·20일 지평의 야간에도 장중에도 유지를 이기는 신호가 없다. 하이닉스 네 조합 전부 동률. 잠금 미개봉
  (유지를 못 이기는 후보에 1회뿐인 잠금을 쓰지 않는다). 같은 특징을 **다음 날 방향 대표 모델**에 더한 R02c
  (`experiments/model_improvement/R02c/20260916T1200Z_group_d_direction/decision.md`, 러너 `run_model_improvement.py --task R02c`)
  는 두 종목 × log_loss·balanced_accuracy·갭 AUC·세션 AUC 열 칸 전부 동률 — 세션 AUC +0.003/−0.003 으로 세션 쪽에도
  더하는 것이 없다. 07:00 특징을 늘리는 방향은 이것으로 소진됐다고 보고, 남은 방향은 시가 이후(09:37) 재예측이다.
- 2026-09-12 실행: R09 0~3단계 완료(`experiments/medium_horizon/R09/decision.md`). 채택 없음,
  관찰로 남김. **지수를 G20 에서 OECD 한국으로 바꾼 것이 이 실험의 전부였다** — 같은 코드·같은
  계약인데 G20 은 2단계부터 전부 동률이고 한국은 세 지평 모두 유의 우위다. 그래도 3단계(이익 모형)
  에서는 두 종목 모두 동률이라 넣지 않았다. 분기가 8개쯤 더 쌓이면(2028년 하반기) 다시 잰다.
  판본 보관은 켰다 — `macro_history/cli_g20_vintages.csv`. 한국 지수는 변화 자기상관이 +0.97 로
  매우 매끄러운데 그 매끄러움 자체가 개정의 결과일 수 있어, 판본이 쌓이기 전까지 2단계의 큰 차이를
  액면 그대로 믿어서는 안 된다.
