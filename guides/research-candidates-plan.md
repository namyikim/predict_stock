...
| R09 | [~] | 선행지수 2~3개월 전망(ARIMA·VAR), 다음 분기 이익 모형에 반영 | 0단계(판본 보관)가 먼저 | 0·1·2단계 완료(2026-09-12). **채택 없음** — ARIMA·VAR 모두 기준선에 동률 이하, VAR 은 세 지평 모두 열위. 1개월에서 기준선 drift 만 rw 를 이긴다. 전망값은 기준선으로 낸다(2026-09 100.293, 10월·11월 100.297, 그림 포함). 3단계는 조건 미충족으로 미실행. 판본 보관은 켰다 |
...
- 2026-09-12 실행: R09 0·1·2단계 완료(`experiments/medium_horizon/R09/decision.md`). 채택 없음.
  ARIMA(2,1,0)·VAR(2) 모두 기준선에 동률 이하이고, VAR 은 세 지평 모두 `drift` 에 열위다.
  3단계는 계획 조건("2단계를 통과한 전망만")을 채우지 못해 하지 않았다.
  판본 보관은 켰다 — `macro_history/cli_g20_vintages.csv`. 판본이 12개월쯤 쌓이면(2027년 하반기)
  개정을 반영한 재측정이 처음으로 가능해진다. 그때 다시 재는 것이 R09 의 남은 일이다.
