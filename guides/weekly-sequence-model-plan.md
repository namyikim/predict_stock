...
상태: S01~S04 완료. S04 실제 스냅샷 판정 — 삼성전자 **미채택**, SK하이닉스 **미채택**(2026-09-22). 운영 채택 없음. S05 이후는 사용자 결정 대기.
...
| S04 | 완료 — 두 종목 미채택 | 실제 데이터 시간순 full 비교; 결과·근거는 guides/weekly-sequence-s04-implementation.md '결과' 절 |
| S05 | 보류 | 과거 잔차 기반 가격 범위 및 입력 그룹 제거 비교; 기여도와 인과 구분 — S04 가 미채택이라 진행 여부는 사용자 결정 |
...
- S04 코드 (2026-09-22, Claude): 사용자가 A(Colab) 경로를 택함. 상세 계획 작성 후 구현, main 2fca009a.
  ...
  - 운영 Ridge 재현: forecast_utils.make_price_model + price_design_frame + sigma 스케일을 그대로 쓰되,
    ...
    동일 폴드에서 M00 계산 경로와 raw 예측·MAE가 1e-9 이내로 일치하는 회귀 테스트를 완료했다.
    과거 실제 M00 summary와 S04 결과는 폴드 정의가 달라 직접 비교하지 않는다.
  ...
  - 다음: Colab 에서 노트북 실행(samsung → sk_hynix). 결과가 오면 두 종목 판정 표를 설계 문서에 적고
    채택 후보만 S05 로 간다.
- S04 실행·판정 (2026-09-22, Colab full, Claude 기록): 두 종목 모두 **미채택**. 상세 표와 해석은
  guides/weekly-sequence-s04-implementation.md '결과' 절.
  - 실제 자료가 드러낸 것 둘: ① 야후 봉에 고가·저가 밖의 시가·종가가 있다(삼성 1봉, 하이닉스 3봉 — 그중 둘은
    거래량 8천·1만 주로 정상 세션이 아님). 값을 고치지 않고 결측 처리하고, 불가능한 봉이 1% 를 넘으면 거부
    (크기 문턱은 근거가 없어 쓰지 않음). ② Colab 은 numpy 가 미리 올라와 있어 고정 버전 설치 후 런타임
    재시작이 필요하다 — 노트북을 2회 실행 구조로 바꿨다.
  - 방법이 제 몫을 한 곳: seed 셋 보고(삼성 잠금 seed 편차가 모델 차이보다 컸다), 개발+잠금 둘 다 기준
    (하이닉스 잠금 우위를 우연/국면과 가르지 못하게 막았다), 공통 예측일(운영 Ridge 와 시퀀스 날짜 차이).
  - 운영 Ridge 회귀 검증 완료(2026-09-22): 합성 고정 입력에서 기존 M00 계산 경로가 만든 동일 폴드의
    raw 예측·MAE와 S04 연결 결과가 허용오차 1e-9 이내로 일치한다. 과거 실제 summary와 S04 수치는
    폴드 정의가 달라 직접 수치 비교하지 않는다.
  - 잠금 12개월은 두 종목 모두 열렸다(LOCK_OPENED_samsung.json, LOCK_OPENED_sk_hynix.json). 다시 열지 않는다.
  - 다음 단계 후보(사용자 결정): (a) 월 1회 개발 폴드만 재평가하는 워크플로, (b) 종료. 어느 것도 운영
    채택으로 바로 이어지지 않는다 — 운영 채택에는 새 사전 기준과 아직 보지 않은 새 평가 구간이 필요하다.
- 세션 메모 (2026-09-22): 사용자 API 토큰이 만료되어 임시 크레딧으로 이 기록까지 마쳤다. 토큰이 갱신되면
  위 '다음 단계 후보' 중 하나를 골라 재개한다. 재개 첫 명령:
  `python -m unittest tests.test_weekly_sequence tests.test_weekly_sequence_s04 -v` (환경 확인) 후
  guides/weekly-sequence-s04-implementation.md '결과' 절 읽기.
