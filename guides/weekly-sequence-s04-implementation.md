...
- [ ] 회귀: 기존 M00 quick summary의 raw MAE와 새 함수 결과가 같은 폴드에서 일치(허용 1e-9). — **미실행.** Colab 실행 때 대조하지 못했다(운영 Ridge 개발 MAE 삼성 0.02707·하이닉스 0.04348 은 M01 결과와 자릿수는 맞으나 폴드 정의가 달라 직접 비교는 아니다). 다음 세션의 첫 작업 후보.
...
**S05(운영 병행 발행)는 진행하지 않는다.** 하이닉스 TCN 은 문서상 '관찰 후보' 성격이나 규칙상 판정은 미채택이다. 다음 단계 후보: 월 1회 개발 폴드만 재평가(잠금은 다시 열지 않는다), 또는 운영 Ridge 회귀 대조(위 미실행 항목).
