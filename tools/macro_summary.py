...
태도를 지킨다.
- 판정(안정/주의/위험)을 내리지 않는다. '지금 어디쯤인가'까지만 말한다.
...
def item(label, series, now, unit, flat, fmt="{:,.0f}", reading=""):
    """항목 하나: 이름 · 최근값(기준일) · 위치 · 방향 · (해석). 오래됐으면 지연 표시."""
    stamp, value = _last(series)
    if value is None:
        return {"label": label, "text": "자료 없음", "stale": True, "stamp": None}
    stale = is_stale(series, now)
    head = f"<b>{escape(label)}</b> {fmt.format(value)}{escape(unit)} <span class='muted'>({stamp:%Y-%m} 기준)</span>"
    if stale:
        return {"label": label, "stamp": stamp, "stale": True,
                "text": head + " — <span style='color:#a8322a'>업데이트 지연</span>. 이 값은 아래 판단에 쓰지 않습니다."}
    text = f"{head} · {where_word(position(series))} · {trend_word(direction(series), unit, flat)}"
    if reading:
        text += f". {escape(reading)}"
    return {"label": label, "stamp": stamp, "stale": False, "text": text,
            "pos": position(series), "chg": direction(series)}
...
def build_items(fx, us_jp, us_market, now):
    ...
    if fx is not None:
        items.append(item("원/달러", fx.get("usdkrw"), now, "원", 15,
                          reading="원/달러가 오르면 원화가 약해진 것입니다."))
        items.append(item("미 달러지수", fx.get("dxy"), now, "", 1.0, fmt="{:.1f}",
                          reading="여러 통화 대비 달러의 힘입니다. 원/달러 결정 요인 표에서 가장 큰 몫을 차지해 왔습니다."))
        items.append(item("한·미 10년물 금리차", fx.get("rate_gap"), now, "%p", 0.15, fmt="{:+.2f}",
                          reading="한국 − 미국. 흔히 이 값이 낮을수록 원화가 약해지는 쪽으로 읽힙니다."))
        items.append(item("한·미 실질금리차", fx.get("real_rate_gap"), now, "%p", 0.15, fmt="{:+.2f}",
    ...
    if us_market is not None:
        ...
        items.append(item("미국 10년물", us_market.get("us10y"), now, "%", 0.2, fmt="{:.2f}",
                          reading="장기 금리. 오르면 주식·장기 자산에 흔히 부담으로 읽힙니다."))
        items.append(item("나스닥", us_market.get("nasdaq"), now, "", 400, fmt="{:,.0f}"))
    return items
...
def summary_html(fx, us_jp, us_market, now):
    ...
    rows = "".join(f'<li style="margin:7px 0;line-height:1.7">{it["text"]}</li>' for it in items)
    ...
    stale_html = (f'<div style="font-size:12px;color:#a8322a;margin-top:8px">업데이트 지연: '
                  f'{escape(", ".join(stale))} — 이 지표는 위 관찰점에 쓰지 않았습니다.</div>' if stale else "")
    return (
        ...
        '<div style="font-size:11px;color:#7a8797;margin-top:4px">위치 = 최근 12개월 범위 안에서 어디인가 · '
        '방향 = 최근 3개월 변화 · 해석은 흔히 쓰이는 읽는 법이지 지금의 인과 주장이 아닙니다.</div>'
        '<div style="font-size:13px;font-weight:700;margin-top:12px">지금 어디쯤인가</div>'
